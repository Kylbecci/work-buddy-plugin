...
- **Windows** — Claude Code matches the path as the **tilde form with backslashes** (`~\.claude\work-buddy\...`). This is the form that actually matches — **confirmed on a live Windows install (2026-06-17)**. The forward-slash tilde form (`~/.claude/work-buddy/**`) and absolute drive forms (`C:/Users/...`, `//c/c/Users/...`) do **not** match on Windows — don't bother emitting them. **Emit BOTH a double-star and a single-star form for each of Read / Edit / Write:**
  - `Read(~\.claude\work-buddy\**)`, `Edit(~\.claude\work-buddy\**)`, `Write(~\.claude\work-buddy\**)` — the `\**` form matches files in **subfolders** (`logs\…`, `recaps\…`, `meetings\…`).
  - `Read(~\.claude\work-buddy\*)`, `Edit(~\.claude\work-buddy\*)`, `Write(~\.claude\work-buddy\*)` — the `\*` form matches files **directly in** the work-buddy folder (`tasks.md`, `config.md`, `context.md`, `triage-heuristics.md`). **Required** — on Windows the `\**` form does **not** cover direct children, so without the `\*` form the top-level files still prompt on every write (**confirmed on a live Windows install, 2026-06-17**).
  - **In `settings.json` (JSON), each backslash must be escaped** — write `"Edit(~\\.claude\\work-buddy\\**)"` and `"Edit(~\\.claude\\work-buddy\\*)"`, which store as `Edit(~\.claude\work-buddy\**)` and `Edit(~\.claude\work-buddy\*)`.
...
> **Existing Windows installs:** anyone set up before 2026-06-17 has the old forward-slash-only rules (or only the `\**` form) and will still be prompted on some or every work-buddy write — top-level files like `tasks.md` always prompt without the `\*` form. They must **re-run the permissions step** to get both backslash tilde forms. (Updating the plugin does not rewrite already-written rules.)
...
2. Build the rule list: `Bash(date:*)` + the read-only Bash allows (`Bash(ls:*)`, `Bash(find:*)`, `Bash(cat:*)`, `Bash(grep:*)`, `Bash(wc:*)`, `Bash(head:*)`, `Bash(tail:*)`) + the data-file rules in the **OS-correct form** (see *Path form by OS* — on Windows BOTH the `~\.claude\work-buddy\**` and `~\.claude\work-buddy\*` backslash tilde forms, escaped as `~\\.claude\\work-buddy\\**` and `~\\.claude\\work-buddy\\*` in JSON; on macOS/Linux the forward-slash tilde form) + one rule per detected, connected connector.
